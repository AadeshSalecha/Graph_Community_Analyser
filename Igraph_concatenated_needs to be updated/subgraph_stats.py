...
def B_C_degree_stats(tweet_id, cluster, graph):
  ...
  with open(tweet_id + '/B_C_degree_stats_l1_' + tweet_id + '.txt', 'w') as f:
    for cl in cluster:
      ...
      ''' in-degree..'''
      in_d_boundary_nodes = igraph.degree(graph = graph, v = list(boundary_nodes), mode = 'in')
      ...
      in_d_core_nodes = igraph.degree(graph = graph, v = list(core_nodes), mode = 'in')
      ...
      ''' out-degree..'''
      out_d_boundary_nodes = igraph.degree(graph = graph, v = list(boundary_nodes), mode = 'out')
      ...
      out_d_core_nodes = igraph.degree(graph = graph, v = list(core_nodes), mode = 'out')
