...
def TSM(graph, vertices, tweet_id):
  ...
  TSM_scores = {}
  TS_file = ''.join([tweet_id, '/New_TSM_igraph_network_l1_', tweet_id, '.txt'])
  with open(TS_file, 'w') as f:
    writer = csv.writer(f)
    for node in vertices:
      TSM_scores[node] = (hti[node], htw[node])
      writer.writerow([node, hti[node], htw[node]])
...
if __name__ == '__main__':
  ...
  TSM(tweet_id = 'sample', graph = G, vertices = list(range(0, 19)))
